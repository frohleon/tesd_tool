...
TES-D was built by combining two approaches popular in very different disciplines. From Machine Learning (ML) and Natural Language Processing (NLP), we borrowed the only recently established practice to carefully examine and document the datasets used for method development and research [@gebru2021datasheets; @bender2018data]. From the social sciences, we took the idea of using error frameworks to systematize the research process as well as the notion of (cumulative) errors (Groves et al. 2011) More specifically, we use the TED-On [@sen2021total] to structure our documentation template and to inspire many of its question.
